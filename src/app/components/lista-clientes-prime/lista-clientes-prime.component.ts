...
@Component({
  selector: 'app-lista-clientes-prime',
  templateUrl: './lista-clientes-prime.component.html',
  styleUrls: ['./lista-clientes-prime.component.scss']
})
export class ListaClientesPrimeComponent implements OnInit {

  clientes: ICliente[] = [];

  pageSize = [{ showAll: "Todos" }, 10, 25, 50, 100];
  searchField = '';
  ...
  novoCliente(){
    this.router.navigate([`/criar-cliente-prime/`]);
  }

  editarCliente(cliente: ICliente){
    this.router.navigate([`/editar-cliente-prime/${cliente.codigo}`]);
  }
